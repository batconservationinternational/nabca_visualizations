...
threatSum <- threats %>% 
  filter(!is.na(sppCode)) %>% 
  group_by(sppCode, category, subcategory, cntry) %>% 
  dplyr::summarise(meanImpact = mean(impact_numeric, na.rm = T), 
                   meanScope = mean(scope, na.rm = T),
                   meanSeverity = mean(severity, na.rm = T),
                   n = n(),
                   sd = sd(impact_numeric, na.rm = T)) %>% 
...
plotThreat <- function(cntry_sel){
  ...
  threatRank <- threatSub %>% complete(threat_abbrev, totImpactScore) %>% filter(totImpactScore == "very high") %>% 
    mutate(noSpp = ifelse(is.na(noSpp), 0, noSpp)) %>%
    arrange(noSpp) %>% pull(threat_abbrev) 
